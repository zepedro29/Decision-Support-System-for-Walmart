...
for (i in 1:num_departamentos) {
  ...
  # Obtenha a série temporal para o departamento atual
  TS_dep <- TS[, dep]
  
  # Inicializar os dados de treino com os dados originais
  TS_dep_train <- TS_dep[1:(length(TS_dep) - num_semanas)]
  
  for (j in 1:num_iteracoes) {
    # Treinamento do modelo ARIMA com os dados de treinamento
    AR <- auto.arima(TS_dep_train)
    
    # Previsões para as próximas 4 semanas
    F <- forecast(AR, h = num_semanas)
    Pred <- F$mean
    ...
    TS_dep_train <- c(TS_dep_train, Pred)
  }
}

# Definir o caminho e o nome do arquivo de saída
output_file <- "C:/Users/jpska/OneDrive/Ambiente de Trabalho/TIAPOSE/Previsoes/ARIMAprevisoes_walmart.csv"
