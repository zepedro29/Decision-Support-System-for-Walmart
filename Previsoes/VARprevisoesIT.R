...
# Função para prever com o modelo VAR e armazenar resultados
prever_var <- function(dep, TS_dep_train, fuel, holiday, iteracao) {
  cdata <- cbind(fuel, holiday, TS_dep_train)
  mtr <- ts(cdata, frequency=K)
  mvar <- autoVAR(mtr, LAGMAX=16)
  FV <- forecastVAR(mvar, h=LTS)
  Pred <- FV[[3]]  # Supondo que o terceiro elemento seja a previsão do departamento
...
# Loop para prever cada departamento individualmente com o modelo VAR
for (dep in departamentos) {
  ...
  for (j in 1:num_iteracoes) {
    iteracao_df <- prever_var(dep, TS_dep_train, fuel, holiday, j)
...
output_file <- "C:/Users/jpska/OneDrive/Ambiente de Trabalho/TIAPOSE/Previsoes/VARprevisoes_walmart.csv"
