...
for (i in 1:num_departamentos) {
  ...
  for (j in 1:num_iteracoes) {
    # Treinamento do modelo ETS com os dados de treinamento
    ETS <- ets(TS_dep_train)
    
    # Previsões para as próximas 4 semanas
    F <- forecast(ETS, h = num_semanas)
    Pred <- F$mean
...
output_file <- "C:/Users/jpska/OneDrive/Ambiente de Trabalho/TIAPOSE/Previsoes/ETSprevisoes_walmart.csv"
