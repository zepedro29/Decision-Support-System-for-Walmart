...
for (i in 4:7) {
  ...
  S <- round(K / 3) # Incremento para o treinamento incremental
  ...
  for (b in 1:Runs) {
    ...
    M2 <- fit(y ~ ., D[H2$tr, ], model = "mlpe") # MLPE model
...
#write.csv(median_metrics, file = "C:/Users/jpska/OneDrive/Ambiente de Trabalho/TIAPOSE/Metricas/MLPEmedian_metrics.csv", row.names = TRUE)
